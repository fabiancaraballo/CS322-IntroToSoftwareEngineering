...
def check():
    ...
    text = flask.request.args.get("text", type=str) #Taken from flask_minijax.py
    ...
    if len(matches) >= flask.session["target_count"]:
        rslt = {"matches": matched, "message": msg, "matching": "You have word 3 or more words in the Anagram!", "anagramFinished": True} #Taken from example()
        return flask.jsonify(result=rslt)
    else:
        rslt = {"matches": matched, "message": msg, "matching": matches, "anagramFinished": False} #Taken from example()
        return flask.jsonify(result=rslt)
