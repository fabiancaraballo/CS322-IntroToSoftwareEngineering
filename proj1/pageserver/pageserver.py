...
def respond(sock):
    ...
    parts = request.split()
    partSplitter = parts[1].strip('/')
    source_path = os.path.join(DOCROOT, partSplitter);
    #print(source_path);
    #print()
    #print("parts == ")
    #print('[%s]' % ', '.join(map(str, parts)))
    #print()
...
def main():
    global DOCROOT 			#Taken from spew.py
    options = get_options()
    port = options.PORT
    DOCROOT = options.DOCROOT		#Taken from spew.py
    if options.DEBUG:
